...
void drawHyJetsMass(double minPt=2.0,double maxPt=3.0)
{
   TFile* f1 = new TFile(Form("hyjetsMassHists_minPt%.1f_maxPt%.1f.root",minPt,maxPt));
   ...
   for(int iy=0; iy<ana::nuOfY; iy++){
      ...
      hMass[iy]->SetAxisRange(2.286-0.15,2.286+0.15,"X");
      hMass[iy]->Draw();
...
//      lgd->AddEntry(hMassCentMtd[iy], "0-10% w/ MTD", "lp");
      lgd->Draw();
      ltx->DrawLatexNDC(0.5, 0.7, Form("%.1f < p_{T} < %.1f GeV",minPt,maxPt));
      ltx->DrawLatexNDC(0.5, 0.6, Form("%.1f < y < %.1f", ana::ybin[iy], ana::ybin[iy+1]));
      c[iy]->Print(Form("figures/hydjetMass_LamC3P_minPt%.1f_maxPt%.1f_y%d.pdf",minPt,maxPt,iy));
      c[iy]->Print(Form("figures/hydjetMass_LamC3P_minPt%.1f_maxPt%.1f_y%d.png",minPt,maxPt,iy));
   }
   ...
   c1->Print(Form("figures/hydjetFrac_LamC3P_minPt%.1f_maxPt%.1f.pdf",minPt,maxPt));
   c1->Print(Form("figures/hydjetFrac_LamC3P_minPt%.1f_maxPt%.1f.png",minPt,maxPt));
