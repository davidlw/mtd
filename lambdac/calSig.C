#include "myAna.h"
void calSig(double minPt=2.0, double maxPt=3.0)
{
   TFile* f1 = new TFile("genPt.root");
   TFile* f2 = new TFile("HEPData-ins1616207-v1.root");
   TFile* f3 = new TFile(Form("lamC3PMassHists_minPt%.1f_maxPt%.1f.root",minPt,maxPt));
   TFile* f4 = new TFile(Form("hyjetsMassHists_minPt%.1f_maxPt%.1f.root",minPt,maxPt));
