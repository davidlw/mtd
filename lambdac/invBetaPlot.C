...
void invBetaPlot(double minPt=2.0, double maxPt=3.0)
{
   bool draw = true;
   bool draw2D = true;
   ...
   TH2F* hdInvBetaProtonVsP = new TH2F("hdInvBetaProtonVsP", "hdInvBetaProtonVsP", 50, 0, 10, 1000, -0.1, 0.1);

   // only talk about LamC3P pt<0.5
   for(Long64_t ientry=0; ientry<t->GetEntries(); ientry++){
      ...
      if(t->flavor == 1 && t->isMtdDau1)
      {
        hdInvBetaPionVsP->Fill(pD1, 1./t->beta1_PV - invBetaPion(pD1));
        hdInvBetaProtonVsP->Fill(pD1, 1./t->beta1_PV - invBetaProton(pD1));
      }
      if(t->flavor == 1 && t->isMtdDau3) 
      {
        hdInvBetaPionVsP->Fill(pD3, 1./t->beta3_PV - invBetaPion(pD3));
        hdInvBetaProtonVsP->Fill(pD3, 1./t->beta3_PV - invBetaProton(pD3));
      }
      if(t->flavor == -1 && t->isMtdDau2) 
      {
        hdInvBetaPionVsP->Fill(pD2, 1./t->beta1_PV - invBetaPion(pD2));
        hdInvBetaProtonVsP->Fill(pD2, 1./t->beta1_PV - invBetaProton(pD2));
      }
      if(t->flavor == -1 && t->isMtdDau3)
      {
        hdInvBetaPionVsP->Fill(pD3, 1./t->beta3_PV - invBetaPion(pD3));
